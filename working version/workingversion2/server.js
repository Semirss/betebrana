...
app.post('/api/queue/add', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (book.available_copies > 0) {
            return res.status(400).json({ error: 'Book is available for direct rental' });
        }
        ...
        const position = queueItems.findIndex(item => item.user_id === userId) + 1;
...
// Add endpoint to process queue when book is returned
...
// Add endpoint to clean up expired queue entries (should be called periodically)
app.post('/api/queue/cleanup', async (req, res) => {
    try {
        ...
        res.json({ success: true, cleaned: result[0].affectedRows });
    } catch (error) {
...
app.post('/api/books/rent', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Check if user is first in queue when book is unavailable
        if (book.available_copies <= 0) {
            const [queueItems] = await pool.execute(`
                SELECT * FROM queue 
                WHERE book_id = ? 
            ...
            if (queueItems.length === 0 || queueItems[0].user_id !== userId) {
                return res.status(400).json({ 
                    error: 'Book not available. Please join the queue.',
                    available: false 
                });
            }
        }
